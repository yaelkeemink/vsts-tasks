...
{
    $uri = ("{0}/_apis/clt/testruns?{1}&detailed=false&{2}" -f $CltAccountUrl, $apiVersion, $filter)
        ...
        if ($runsInLastBuild)
        {
            foreach ($previousRun in $runsInLastBuild)
            {
                if ($previousRun.name -eq $run.name)
                {
                    ...
                    ('| [Compare with run {4} from build {0}]({1}/_apps/hub/ms.vss-cloudloadtest-web.hub-loadtest-account?_a=compare&runId1={2}&runId2={3}). |' -f  $lastSuccessfulBuild.id, $TFSAccountUrl, $previousRun.id, $run.id, $previousRun.runNumber) >>  $summaryFile
                    break
                }
            }
        }           
